...
const Jellyfish = ({ size, opacity }: { size: number; opacity: number }) => {
    ...
    <svg width={size} height={size * 1.6} viewBox="0 0 60 96" fill="none" style={{ opacity }}>
      ...
      {Array.from({ length: tentacleCount }).map((_, i) => {
        ...
        return (
          <motion.path
            key={i}
            d={`M ${x} 40 C ${cp1x} 58, ${cp2x} 72, ${x + (Math.random() - 0.5) * 8} 90`}
    ...
    <section className="relative h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        {jellyfishData.map((jf, i) => (
          ...
          >
            <Jellyfish size={jf.size} opacity={jf.opacity} />
          </motion.div>
        ))}
      </div>
