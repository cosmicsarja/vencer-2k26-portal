      ...
      <div className="container px-4" ref={ref}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
          {stats.map((s, i) => (
            ...
            >
              <s.icon className={`mx-auto mb-3 ${s.color}`} size={32} />
              <div className="font-display text-2xl sm:text-3xl text-foreground mb-1">{s.value}</div>
              <div className="font-heading text-sm uppercase tracking-widest text-muted-foreground">{s.label}</div>
            </motion.div>
          ))}
        </div>
      </div>
